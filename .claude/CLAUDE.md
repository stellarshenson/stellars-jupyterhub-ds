...
All bootstrap decision logic lives in `duoptimum_hub_services.admin_bootstrap` as pure, unit-tested functions (`query_admin_state`, `bootstrap_window_open`, `admin_unreachable`, `provision_admin_userinfo`, `first_admin_self_signup_pending`); `config/jupyterhub_config.py` only wires them and raises on the unreachable case. Exactly one path creates the first admin, decided deterministically at startup from `query_admin_state(JUPYTERHUB_ADMIN)` - a raw sqlite read of `users_info` returning the DB-empty and admin-present flags. `JUPYTERHUB_ADMIN` is lowercased at load to match JupyterHub's username normalisation, so every raw comparison below holds.
...
**Admin promotion**: in every path, first login fires `post_auth_hook`, which sets `authentication['admin']=True` for `JUPYTERHUB_ADMIN`.
