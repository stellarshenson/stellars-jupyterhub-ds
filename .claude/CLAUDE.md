...
- Admin role is granted at login via `post_auth_hook` matching `JUPYTERHUB_ADMIN_USERNAME` (no `c.Authenticator.admin_users` declaration; that would eagerly create the admin User row at startup and trigger random-password auto-creation in `events.py`)
...
All bootstrap decision logic lives in `duoptimum_hub_services.admin_bootstrap` as pure, unit-tested functions (`query_admin_state`, `bootstrap_window_open`, `admin_unreachable`, `provision_admin_userinfo`, `first_admin_self_signup_pending`); `config/jupyterhub_config.py` only wires them and raises on the unreachable case. Exactly one path creates the first admin, decided deterministically at startup from `query_admin_state(JUPYTERHUB_ADMIN_USERNAME)` - a raw sqlite read of `users_info` returning the DB-empty and admin-present flags. `JUPYTERHUB_ADMIN_USERNAME` is lowercased at load to match JupyterHub's username normalisation, so every raw comparison below holds.

**Env-provision (`JUPYTERHUB_ADMIN_PASSWORD` set)**: `provision_admin_userinfo()` runs inside `BootstrapAdminAuthenticator.__init__`, AFTER `super().__init__()` - NativeAuthenticator's `add_new_table()` has created `users_info` by then, so the insert is deterministic with no sqlite race (the previous config-load timing hit a missing table on a fresh volume and silently no-op'd, which was the admin-login bug). Using the authenticator's own ORM session it inserts the admin `UserInfo` with `bcrypt(JUPYTERHUB_ADMIN_PASSWORD)` and `is_authorized=True` only when the admin row is absent, logging one line that the initial password was provided and the admin is being created (NEVER the password). Initial-only: if the row exists and `is_valid_password(env)` still matches it is a no-op; once it no longer matches (admin changed their password in the UI) the env is permanently ignored. `JUPYTERHUB_ADMIN_PASSWORD` is intentionally absent from `settings_dictionary.yml` so it is never shown on the Settings page.

**Signup-off bootstrap window (no env password, signup off)**: `bootstrap_window_open` is true iff signup is off (`JUPYTERHUB_SIGNUP_ENABLED=0`), no env password, `users_info` empty, and no admin row. While open, `BootstrapAdminAuthenticator.validate_username` rejects every username except `JUPYTERHUB_ADMIN_USERNAME`, so the admin can self-register at `/hub/signup` even though signup is otherwise off; `allow_self_approval_for` self-approves them. Once the admin row appears the window closes permanently and `enable_signup` falls back to the operator's `JUPYTERHUB_SIGNUP_ENABLED`.
...
**Fail-fast**: `admin_unreachable` is true when signup is off, no env password, no admin row, and the DB is NOT empty (so the window cannot open) - the admin can never be created, so config raises `SystemExit` with a clear message rather than booting into a locked-out state.

**Admin promotion**: in every path, first login fires `post_auth_hook`, which sets `authentication['admin']=True` for `JUPYTERHUB_ADMIN_USERNAME`.
