...
- **Action buttons** - one class per button, two axes: context sets size, variant sets colour, never stacked. Contexts `page` (form / page-head, the Save baseline), `list` (dense table-row, more compact), `input` (inline with a field), `list-icon` (icon-only square). Variants `primary` (filled accent), `secondary` (bordered neutral), `dangerous` (red), `disabled` (muted, inert) - the `input` context swaps `dangerous` for `warning` (amber), and its buttons match the field height and almost blend in (content can be text, icon+text or icon-only). The old `btn`/`btn-sm`/`btn-primary`/`btn-danger`/`btn-ghost`/`icon-btn` vocabulary is retired. Change-password on the Users list is `list-secondary` (no icon); Stop / Cancel-spawn / Remove / Delete are `list-icon-dangerous`
...
- **Lab Container** - full width; a `Set lab image` action (input-primary) pulls + verifies the image (spinner) then shows a green success line under the field; the volume table lists the standard `jupyterhub_shared` mount as a locked read-only row plus editable custom mounts (home/workspace/cache stay platform-managed and unlisted, per a Note); disabled inputs render explicitly inert (dashed, muted)
- **Server hero (two panels, reused for every user incl. admin)** - the user launchpad and the admin Overview open with one two-panel block: left is identity (`jupyterlab-<user>` + status pill + activity meter, no server icon) plus controls (Open / Restart / Stop) plus a TTL gadget (a progress bar of time-left and an Extend button); right is the lab's resources. Resources read against host capacity (memory as % of host), not per-user limits
