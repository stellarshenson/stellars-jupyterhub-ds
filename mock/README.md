...
- **Action buttons** - one class per button: context (size) x variant (colour), never stacked. Contexts: `page` (form / page-head, the Save baseline), `list` (dense table-row action, more compact), `input` (inline with a field), `list-icon` (icon-only square). Variants: `primary` (filled accent CTA), `secondary` (bordered neutral), `dangerous` (red), `disabled` (muted, inert). E.g. Save = `page-primary`, Change password = `list-primary`, Generate = `input-secondary`, Stop = `list-icon-dangerous`
