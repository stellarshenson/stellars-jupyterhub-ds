...
def test_host_status_bars_have_tooltips(admin_portal, base_url, admin_api):
    ...
    for label in ("CPU", "Memory"):
        row = card.locator(".doh-res-row").filter(
            has=page.locator(".doh-res-label", has_text=label)
        ).first
        expect(row).to_be_visible()
        tip = (row.locator(".doh-res-val").get_attribute("title") or "").strip()
...
def test_gpu_hidden_when_disabled(admin_portal):
    ...
    card = _host_card(page)
    assert card.locator(".doh-res-label", has_text="GPU").count() == 0, "GPU row shown while GPU is disabled"
    assert page.locator(".doh-gpurows").count() == 0, "GPU bars shown while GPU is disabled"
...
def test_gpu_hidden_when_sidecar_down(admin_portal, base_url, admin_api, docker_client):
    ...
    assert page.locator(".doh-gpurows").count() >= 1, "expected the GPU row to show while connected"
