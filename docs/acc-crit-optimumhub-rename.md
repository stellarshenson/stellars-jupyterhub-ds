# Acceptance Criteria - optimumhub service + image rename

The hub's Docker Compose service is renamed `jupyterhub` -> `optimumhub` and the published image `stellars/stellars-jupyterhub-ds` -> `stellars/optimumhub`, so the deployment matches the Optimum Hub branding and the DockerHub push targets the new repo. The hub's URL prefix (`/jupyterhub`, `JUPYTERHUB_BASE_URL`) and all `JUPYTERHUB_*` env vars are unchanged - only the compose service identity and the image tag move. Verified against the code 2026-06-18.

## Compose service rename

- [x] **Service key** - `compose.yml` hub service is `optimumhub` (was `jupyterhub`)
  - log: 2026-06-18 operator: "rename jupyterhub service to optimumhub"
- [x] **depends_on updated** - traefik and watchtower `depends_on` point at `optimumhub` (a stale `jupyterhub` reference would fail compose validation)
  - log: 2026-06-18 both blocks renamed
- [x] **Traefik identifiers** - router/service/middleware renamed `jupyterhub-rtr`/`jupyterhub-svc`/`jupyterhub-ratelimit` -> `optimumhub-*`, consistently in `compose.yml` and the wrapper override
...
- [x] **container_name** - the literal suffix is `-optimumhub` (`${COMPOSE_PROJECT_NAME:-…}-optimumhub`)
  - log: 2026-06-18 renamed; project-name default unchanged
- [x] **Hub bind/connect host** - `c.JupyterHub.hub_ip` and `hub_connect_url` in `config/jupyterhub_config.py` use `optimumhub`; the hub binds to, and CHP / spawned labs reach the hub by, the compose service name
  - log: 2026-06-18 the first rebuild crashed boot with `getaddrinfo ENOTFOUND jupyterhub` (hub_ip still hardcoded the old name); fixed both lines to `optimumhub`

## Image rename

- [x] **Image tag** - the hub image is `stellars/optimumhub` everywhere it is built, tagged, pulled or referenced: Makefile (`HUB_IMAGE`, build `--tag`, `tag`, push, success banners), `compose.yml`, the functional compose, `start.sh`, `start.bat`
  - log: 2026-06-18 operator: "change the image name ... to optimumhub ... so the dockerhub push won't blow up"; chose `stellars/optimumhub`
- [x] **README DockerHub badges** - image-size and pulls badges point at `stellars/optimumhub`
...
- [x] **Functional harness renamed** - the service is `optimumhub` in all three harness compose files; `conftest.py` `BASE_URL`/`HUB_HOST` default to `optimumhub`; the Makefile `--wait`/`restart` targets name `optimumhub`
  - log: 2026-06-18 operator: "fix the tests and harness"
- [ ] **Functional suites pass post-rebuild** - `make test-functional` and `make test-functional-env` are green against the rebuilt `stellars/optimumhub:latest` image
