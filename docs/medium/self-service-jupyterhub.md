...
**Environment management.** This is where the platform departs most dramatically from vanilla JupyterLab. The base conda environment ships with NumPy, Pandas, Polars, SciPy, Scikit-learn, and Matplotlib - 43 packages ready on first login. For deep learning, TensorFlow and PyTorch live in separate on-demand conda environments that avoid dependency conflicts between frameworks. Install them through the Lab Utilities menu.

But the real game-changer is how environments are discovered and managed. A custom extension called `nb_venv_kernels` scans the entire system for conda, venv, and uv environments and registers each as a Jupyter kernel automatically. Create an environment anywhere - via conda, uv, or plain venv - and it appears as a selectable kernel in JupyterLab within seconds. No manual kernel registration, no JSON spec files, no restarts. JupyterLab's built-in plugins for conda and pip management let users install packages directly from the UI. The launcher integrates environment selection with project navigation, so switching between projects and their associated environments is seamless.

Local environments persist in user volumes between server image refreshes. When the admin pushes a new image via Watchtower, user-created conda environments and venv directories survive because they live on persistent volumes. The base image gets updated, user customizations stay intact.

For new projects, the platform includes a standardized [copier-data-science](https://github.com/stellarshenson/copier-data-science) template accessible through Lab Utilities. It scaffolds a complete data science project structure - data directories, notebook directories, source code layout, configuration files - giving teams a consistent starting point instead of each person inventing their own folder structure.
