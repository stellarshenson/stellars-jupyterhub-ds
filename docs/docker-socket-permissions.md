...
Rules enforced server-side (`duoptimum_hub_services.groups_config.validate_docker_selection`) and client-side in the Groups UI:
...
**Implementation**: `services/jupyterhub/duoptimum_hub_services/duoptimum_hub_services/hooks.py::pre_spawn_hook` reads the resolved config and applies the volume mount / sidecar / `privileged=True` accordingly. Changes require a server restart (stop/start cycle).
