...
Rules enforced server-side (`stellars_hub.groups_config.validate_docker_selection`) and client-side in the Groups UI:
...
**Implementation**: `services/jupyterhub/stellars_hub/stellars_hub/hooks.py::pre_spawn_hook` reads the resolved config and applies the volume mount / sidecar / `privileged=True` accordingly. Changes require a server restart (stop/start cycle).
