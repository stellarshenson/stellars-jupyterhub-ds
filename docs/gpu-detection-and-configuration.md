...
- Detection runs once at hub startup, in `resolve_gpu_mode()` (`optimum_hub_services/gpu.py`)
...
A user can belong to several groups. Their effective GPU access is collapsed by `resolve_group_config()` (`optimum_hub_services/group_resolver.py`):
...
The pre-spawn hook (`optimum_hub_services/hooks.py`) translates the resolved selection into a Docker device request plus the `NVIDIA_VISIBLE_DEVICES` selector env on the user container:
...
Group GPU settings persist in the group configuration store (`optimum_hub_services/groups_config.py`): `gpu_access` (bool), `gpu_all` (bool, default true), and `gpu_device_ids` (list of index strings). They take effect on each user's next spawn.
