...
- **Lifecycle** (`gpuinfo_sidecar.py`) - the hub self-starts the sidecar over the docker socket (recreate-fresh every boot from the current image) and removes it at exit, so it works the same regardless of which compose project launched the hub
- **Env**: `JUPYTERHUB_GPUINFO_URL` (default `http://gpuinfo-nvidia:8000`), `JUPYTERHUB_GPUINFO_NETWORK_NAME`, `JUPYTERHUB_GPUINFO_NVIDIA_IMAGE`, `JUPYTERHUB_GPU_UTIL_UPDATE_INTERVAL`
