...
- **Lifecycle** (`gpuinfo_sidecar.py`) - the hub self-starts the sidecar over the docker socket (recreate-fresh every boot from the current image) and removes it at exit, so it works the same regardless of which compose project launched the hub. The sidecar is also DECLARED as a profiled compose service (`gpuinfo` profile, never auto-started) so `docker compose build` produces its image locally; the hub joins the compose-declared `hub_gpuinfo_network` (discovered by its `duoptimum-hub.network.role=gpuinfo` label) rather than creating it. The sidecar image bakes its own runtime spec (NVIDIA env, port, command); the hub does not re-declare it, only orchestrates network/name/labels and applies the nvidia runtime conditionally (portable to non-GPU hosts). The hub-created container carries `duoptimum-hub.container.role=gpuinfo`
- **Env**: `JUPYTERHUB_GPUINFO_NVIDIA_CONTAINER_NAME` (default `gpuinfo-nvidia`), `JUPYTERHUB_GPUINFO_NVIDIA_URL` (default `http://{hostname}:8000`, where `{hostname}` is filled at boot with the address the hub discovers for the running sidecar), `JUPYTERHUB_GPUINFO_NVIDIA_IMAGE`, `JUPYTERHUB_GPU_UTIL_UPDATE_INTERVAL`. The hub-to-sidecar network defaults to the `{network}` token (resolved at boot to the `duoptimum-hub.network.role=gpuinfo` net); set `JUPYTERHUB_GPUINFO_NETWORK_NAME` to a literal name to override
