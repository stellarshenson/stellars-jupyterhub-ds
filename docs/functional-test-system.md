...
- **Two services** in `tests/functional/compose.functional.yml` - the real `stellars/duoptimumhub:latest` hub and a Playwright runner (`mcr.microsoft.com/playwright/python`), both on a test-only network
