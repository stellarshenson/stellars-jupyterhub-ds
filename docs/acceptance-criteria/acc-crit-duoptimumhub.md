...
- **AC-4a** Given an enabled pool, when the admin clicks "Import Keys", then a popup opens to choose one text file; the file is parsed and validated immediately on selection (nothing is added yet), and the admin then commits - Import appends the parsed keys to the pool (success toast with imported count, plus skipped count when any), Cancel adds nothing. Import only - there is no export
  - log: 2026-06-21 #411 added Import button + `parseApiKeysFile` in `GroupPolicyTab.tsx`; functional `test_api_keys_import.py`
  - log: 2026-06-21 reworked into a validate-then-submit popup (operator: "make import a popup that ... immediately validate ... submit / cancel")
- **AC-4b** Given the import popup, when a file is parsed, then keys are taken one per non-blank line - `single`: the whole trimmed line is the key; `pair`: `id,secret` or `id secret` (comma or whitespace) splits into id+secret; blank lines are ignored and lines that do not match the mode are skipped and counted
  - log: 2026-06-21 parsing unchanged from #411, now driven by the popup
- **AC-4c** Given a parsed file, when at least one valid key is found, then the popup previews the valid count (and skipped count when any) and enables Import; when no valid key is found (empty, all-blank, or all mode-mismatched) or the file cannot be read, then the popup states the failure reason ("Import failed: ...") and keeps Import disabled so nothing is added
  - log: 2026-06-21 added
- **AC-4d** Given the API keys pool section, when no import is in progress, then the expected file format is described only inside the import popup, not in the main section
  - log: 2026-06-21 added (operator: "the text about import format - not in the main section")
- **AC-4e** Given the API keys pool section, when it is shown, then the "Add Key" and "Import Keys" buttons each carry a tooltip describing their action
  - log: 2026-06-21 added (operator: "buttons on the group section (add key, import keys) must have tooltips")
...
- [x] **Edge: route-colliding username reserved** ([DEF-31](defects-duoptimumhub.md#def-31-user-names-colliding-with-static-users-routes-new-bulk-not-reserved)) - a user named `new` or `bulk` collides with the SPA's static `/users/new` and `/users/bulk` routes the same way; such names are rejected at creation (signup and admin user-create both call `validate_username`)
  - log: 2026-07-09 met - `DuoptimumNativeAuthenticator.validate_username` (`auth.py`) rejects `_RESERVED_USERNAMES = {new, bulk}` case-insensitively; unit `test_authenticator_bootstrap.py`, functional `test_reserved_username_rejected` (POST /hub/api/users/new|bulk -> 400)
- [x] **Edge: config API 404 on missing group** ([DEF-30](defects-duoptimumhub.md#def-30-groupconfig-getput-fabricate-a-phantom-config-for-a-non-existent-group)) - GET/PUT of a group's config for a name with no `orm.Group` row returns 404 instead of fabricating a phantom config; mirrors the delete handler
  - log: 2026-07-09 met - shared `_require_group` guard on `GroupsConfigHandler.get`/`.put` (`handlers/groups.py`); unit `test_imports.py::test_require_group_404_on_missing`, functional `test_group_config_api_404_on_missing_group`
- [x] **Edge: reserved admin username fails fast** ([DEF-31](defects-duoptimumhub.md#def-31-user-names-colliding-with-static-users-routes-new-bulk-not-reserved)) - setting `JUPYTERHUB_ADMIN_USERNAME` to a reserved name (`new`/`bulk`) raises `SystemExit` at startup instead of silently locking the admin out of `validate_username`
  - log: 2026-07-09 met (bug-hunter/architect review of DEF-31) - fail-fast in `DuoptimumNativeAuthenticator.__init__`; unit `test_authenticator_bootstrap.py::test_reserved_admin_username_fails_fast`. Conscious trade-off: a reserved admin name now crash-loops the hub (loud, `docker logs`-diagnosable) rather than locking out just that admin, matching the existing `admin_unreachable` fail-fast
...
- [x] **Soft pulsing halo (slow connected, 3x faster down)** - the diode has a solid core and a soft halo ring on `::after` that pulses on OPACITY ONLY (no scale - per the design artifact, severity is carried in CADENCE, not amplitude): connected = `doh-pulse-calm` (a gentle opacity dip 1->.6) at the full config period (`ANIMATION.statusPulseMs` = 3.6s, `services/config.ts`, via `--doh-status-pulse`); down = `doh-pulse-alert` (a deeper opacity dip 1->.25, floored above zero so the solid core stays lit - an urgent pulse, not a strobe) at 3x the speed (`calc(var(--doh-status-pulse) / 3)` = 1.2s); the mobile down-panel diode matches the down keyframe + rate
  ...
  - log: 2026-06-22 operator "it is so subtle that I don't see it, must be stronger between low and high" + "the green diode needs to go down just a bit and go back; the warning diode needs to be stronger difference still" - split the single `doh-soft-pulse` into `doh-pulse-calm` (gentle) and `doh-pulse-alert` (strong); amplitude now encodes severity alongside speed. `test_connection_pulse.py` asserts the calm vs alert keyframe names + the 3x ratio
  - log: 2026-06-22 `/adversarial-review` (ux) MAJOR - alert at opacity 1->.05 + scale 1->2 read as a strobe/crash (near-total extinction at ~1Hz) and scale 2 risked clipping the mobile title; softened to .15 / 1.7 (dip floored above zero so the solid core stays lit), still a clearly stronger difference than the calm pulse
- [x] **Healthy state** - while reachable the pill is calm (success tone, "Connected") with a SLOW soft-halo pulse on the diode; low-key, not attention-grabbing
...
- [x] **Down text uses the standard warning colour** - the down/warning indicator text uses `var(--color-warning)` (the standard amber token), not the brighter neutral `--color-text` it carried before; the desktop down pill (`.doh-conn-pill.down`) and the mobile panel title (`.doh-hub-warn-title`) both use it; the pale 14% warning fill keeps the amber legible
  - log: 2026-06-22 operator "no connection text on the warning indicator should use standard warning text colour, not the brighter one"; `--color-text` -> `--color-warning` on the down pill text + mobile panel title
  - log: 2026-06-22 `/adversarial-review` (ux) flagged light-theme amber-on-pale-amber ~4.06:1 (below AA 4.5:1 for xs/600 text); kept per the operator's explicit "standard warning colour" instruction (dark theme ~4.9:1 passes); revisit (darken the light warning token or restore neutral text) if strict AA is required
...
- [x] **Reduced motion** - under `prefers-reduced-motion` the diode halo pulse is disabled but a steady halo (`opacity:.6`, no transform) is kept so the diode still reads as live; the `@media` rule targets `…::after`
  - log: 2026-06-21 requirement carried
  - log: 2026-06-22 verified by `test_connection_pulse.py` (under emulated `reduce`, the `::after` animation-name is `none`)
- [ ] **Edge: recovery clears it** - when the probe flips back to up, the pill returns to healthy and the mobile panel disappears immediately (driven by `down` / `downSince`)
  - log: 2026-06-21 requirement carried
- [x] **Visual sign-off** - desktop + mobile screenshots of the down state reviewed against the above (warning-not-error, unobtrusive pill, soft slow pulse, elapsed present, pale mobile bg); operator sign-off recorded
  ...
  - log: 2026-06-21 reviewed 3 screenshots (healthy-desktop, unreachable-desktop, unreachable-mobile) - layout/colour/hierarchy/non-blocking/elapsed/pale-mobile-bg all pass; mobile body copy in the captured build was stale (pre-#423 "The Duoptimum Hub..."), corrected terse/branded copy verified in source (`hubName()` lead, both surfaces) - a post-rebuild re-shot would capture it live; operator signed off
...
- [ ] **Home overview drops the side column on tablet** - below xl (1200) the admin Home drops the Quick Actions + Recent Events side column and the Active Servers list runs full-width; at/above xl the two-column `1.6fr 1fr` layout is unchanged; defaults to the wide layout until `Grid.useBreakpoint` resolves so a desktop never flashes the narrow view
  - log: 2026-06-26 operator "quick actions and events widget can be also turned off as part of the responsive design - leaving servers list"; `AdminHome` `sideColumn = !known || screens.xl`; tsc + build:hub green; verify pending live rebuild
- [ ] **Home Active-Servers preview reduces columns** - the home preview list keeps User / Status / CPU / Mem / Time Left / actions always; Activity drops below lg (992) via `useResponsiveColumns`, matching the full Servers list (which drops Activity at the same tier)
  - log: 2026-06-26 operator "then reducing columns in the servers list widget"; `ActiveServersPreview` columns wrapped in `useResponsiveColumns`, Activity `responsive: ['lg']`
