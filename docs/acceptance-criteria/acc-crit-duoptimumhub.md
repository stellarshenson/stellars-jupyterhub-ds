...
- [x] **Bar glows on extend; counter blurs (no counter glow)** - during the boost the bar (`.doh-ttl-bar`) gets a glow around its outer edge (behind the fill); the time readout (`.doh-ttl-val`) does NOT glow - it only blurs .75px at the peak
...
- [x] **Glow is a box-shadow halo UNDER the bar (around, not over)** - the bar glow is a `box-shadow` on the rounded track (`.doh-ttl-bar.doh-ttl-boost .ant-progress-inner`) in the bar's own tone brightened toward white (`color-mix(in srgb, currentColor, white 60%)`); box-shadow draws outside the pill's box and is occluded by the fill, so the glow haloes the OUTER edge and sits UNDER the bar - never over its surface; the track's own `overflow:hidden` does not clip its own box-shadow
  ...
  - log: 2026-06-22 operator "glow shows on top of the progressbar, make it under it - glows around it not over it" - the `drop-shadow` filter traced the fill's alpha and bled the halo over the track at the fill's leading edge; switched to a `box-shadow` on `.ant-progress-inner` (outer halo, behind the fill); radius is the new mechanism's calibration point (3px), may need an operator visual re-tune
...
- [x] **Glow is HELD, not pulsed (trapezoid envelope)** - the glow is driven by a CSS `transition` on the track's `box-shadow`, not a keyframe: ramps ON over `--doh-ttl-glow` (250ms) when `doh-ttl-boost` is set, HOLDS for the whole boost/fill window, ramps OFF over 250ms when the value lands; no mid-animation dip, never a one-shot pulse
