...
- **Unit tests** - `make test` runs the `optimum-hub-services` and `stellars-docker-proxy` pytest suites; they also gate the Docker build and run as a GitHub `unit_tests` job
...
- **Hub limits** (`optimum_hub_services.abuse_protection`): `concurrent_spawn_limit` throttles spawn-storms (container spawns are expensive), `active_server_limit` caps total capacity, and NativeAuthenticator login lockout blocks an account for `JUPYTERHUB_LOGIN_LOCKOUT_SECONDS` after too many failed attempts
