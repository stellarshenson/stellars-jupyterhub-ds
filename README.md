...
[![CI](https://github.com/stellarshenson/stellars-jupyterhub-ds/actions/workflows/docker-build.yml/badge.svg)](https://github.com/stellarshenson/stellars-jupyterhub-ds/actions/workflows/docker-build.yml)
...
- **Hub limits** (`duoptimum_hub_services.abuse_protection`): `concurrent_spawn_limit` throttles spawn-storms (container spawns are expensive), `active_server_limit` caps total capacity, and NativeAuthenticator login lockout blocks an account for `JUPYTERHUB_LOGIN_LOCKOUT_SECONDS` after too many failed attempts
...
- GPU / Docker (standard) / Limited Docker / Privileged: **grants win** - OR across all groups. Once any group grants, no other group can revoke. Standard supersedes limited (raw socket makes the proxy moot); limited quota fields use max-wins
...
> [!WARNING]
> `Standard Docker Access` and `Privileged Docker (root)` grant significant privileges. Standard provides full Docker host control via the unfiltered socket; Privileged provides kernel-root inside the lab including hardware and kernel-module access. `Limited Docker Access` is the safer alternative for users who only need to manage their own containers - it caps quotas and labels every resource for owner-based filtering. Within Limited, the `allow_dangerous_flags` toggle is OFF by default and surfaced with a red warning because flipping it on hands the user kernel-level escape vectors (host binds, host net/pid, cap-add, device passthrough) through the proxy. Only grant these to trusted users.
