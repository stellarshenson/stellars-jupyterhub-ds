...
- **File Downloads**: an on/off grant that exempts a group's members from the platform download block. Effective only when the operator sets `JUPYTERHUB_BLOCK_FILE_DOWNLOADS=1`; then a user may download files from their lab via the browser only if one of their groups has this on (any granting group wins). When blocked, the hub overlays per-user routes onto the lab's download surfaces (`files/?download`, notebook export, the export-markdown and share-files download endpoints), returns a 403, and the user gets a "blocked by policy" notification; inline viewing, the contents API, kernels, and terminals are untouched. This is **best-effort browser-download policy with notification and an audit trail, not full data-loss prevention** - the lab has a root shell with internet egress, so a determined user can still copy data out through a terminal or kernel over an encrypted channel. Takes effect on the member's next server start. See [docs/acc-crit-group-file-downloads.md](docs/acc-crit-group-file-downloads.md)
...
- File downloads: **grant wins** - a user is download-blocked (when the master switch is on) unless ANY of their groups has the File Downloads grant on
