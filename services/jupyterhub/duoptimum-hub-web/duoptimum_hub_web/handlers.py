"""Duoptimum Hub portal handler - serves the React SPA from the hub itself.
...
def _entry_assets():
    ...
    try:
        ...
        _log.warning(
            "[DuoptimumHub] manifest unreadable at %s (%s) - portal shell will be blank until fixed",
            manifest_path, e,
        )
        return ("", "")
    ...
    if not entry or not entry.get("file"):
        _log.warning(
            "[DuoptimumHub] no isEntry chunk in %s - portal shell will be blank", manifest_path,
        )
        return ("", "")
...
class PortalHandler(BaseHandler):
    """Serve the Duoptimum Hub SPA: a bundled file if it exists, else the shell."""
