...
test('capture pages - dark', async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem('duoptimum-hub-theme', 'dark'))
...
test('capture user home + design - light', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('duoptimum-hub-theme', 'light')
    localStorage.setItem('duoptimum-hub-role', 'user')
  })
