# Duoptimum Hub portal - action audit (live mode)

Action-by-action audit of the hub-served React portal (`duoptimum-hub-web`) in LIVE mode (`VITE_DATA_MODE=live`), to record what is genuinely wired to the hub versus what is still mock or broken. Produced by six parallel read-only audit agents, each tracing the full path: view (`src/pages`/`src/components`) -> action layer (`src/services/ops.ts`) or read (`src/services/hub/liveSource.ts`) -> HTTP (`src/services/hub/client.ts`) -> route (`config/jupyterhub_config.py`) -> backend handler.
