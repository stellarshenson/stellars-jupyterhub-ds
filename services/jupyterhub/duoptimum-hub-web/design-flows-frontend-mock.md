# Duoptimum Hub - flows, navigation and screen registry
...
CONTENT-TOP  breadcrumb (Duoptimum Hub / [parent /] page)
SIDER FOOT   identity · sign out
HEADER       language · theme controls (actionsRender)
FOOTER       Duoptimum Hub + JupyterHub versions + stack chips
...
**Flow 3 - configure one user of many** - USR-001 filter by name, click the name (an accent link) -> USR-005 full tabbed screen: **Profile** (username fixed, email, change-password with a Generate button - the shared `genPassword` helper, also on New user and self-service Profile, Administrator + Authorised switches), **Groups** (`GroupPicker` add + browse, with the resolved effective policies), **Volumes** (reset selected, server stopped only). One Save (`FormFooter`). The Authorised toggle is also inline on the list row. The built-in admin (`JUPYTERHUB_ADMIN_USERNAME`) is locked - its role / authorisation / removal are owned by system config, not the screen.
