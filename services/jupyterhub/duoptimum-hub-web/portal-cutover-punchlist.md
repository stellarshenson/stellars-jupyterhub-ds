# Duoptimum Hub - portal cutover punch-list
...
- [x] **L2 - Version single source** - `package.json` aligned to `0.1.0`; `__version__` now reads `importlib.metadata.version("duoptimum-hub-web")` (the wheel/pyproject version) with a fallback
...
- [x] **L5 - Stale artifacts cleared** - removed local `dist/`, `dist-wheel/`, `build/`, generated `duoptimum_hub_web/static/`; `make clean` already covers these. Docker regenerates them in the webbuilder stage
...
- [x] **OP2 - eslint no longer scans the generated bundle** - added `duoptimum_hub_web/static`, `dist-wheel`, `test-results`, `playwright-report` to the eslint flat-config ignores; `npm run lint` is now clean (exit 0) and stays clean after `make assets`
