...
from stellars_hub_services.policy import POLICY_TYPES, ApplyContext, apply_policies
...
class TestDockerApply:
    ...
    def test_limited_registers_and_mounts(self, monkeypatch):
        ...
        monkeypatch.setattr('stellars_hub_services.docker_proxy.register_user', fake_register_user)
...
class TestApiKeysApply:
    def test_assigns_env_and_label(self, monkeypatch):
        ...
        from stellars_hub_services.api_keys_pool import PoolManager, normalize_pool, pool_label_key
    ...
    def test_group_env_var_wins_over_pool(self, monkeypatch):
        from stellars_hub_services import api_keys_pool as akp
        from stellars_hub_services.api_keys_pool import PoolManager, normalize_pool
        PoolManager._instance = None
        monkeypatch.setattr(akp, 'observe_in_use', _empty_observe)
        monkeypatch.setattr(akp, '_container_name', lambda u: f'jupyterlab-{u}')
