...
# ── Scheduled reconciliation (heavy imports done lazily) ─────────────────────

def schedule_startup_api_keys_reconcile():
    """Rebuild the in-use view from running containers on hub startup.

    Mirrors `schedule_startup_docker_proxy_callback`: pre_spawn_hook only fires
    on new spawns, so survivors of a hub restart are reconciled here. Because the
    in-use set is always re-derived from container labels, this is mostly a
    tentative-set reset plus a one-line summary of what is observed in use.
    """
...
def schedule_api_keys_reconcile(interval_seconds):
    """Start the periodic reconcile (self-healing against missed stop events)."""
    ...
    async def _pass():
        try:
            ...
            from jupyterhub.app import JupyterHub
            JupyterHub.instance().log.exception("[ApiKeys] reconcile pass failed")
    ...
    def _start():
        ...
        app.log.info(f"[ApiKeys] periodic reconcile started - interval={interval_seconds}s")
