...
class TestResolveGpuMode:
    def test_mode_0_disabled(self):
        ...
        with patch("optimum_hub_services.gpu.enumerate_gpus") as mock_enum:
    ...
    def test_mode_1_enabled_enumerates(self):
        ...
        with patch("optimum_hub_services.gpu.enumerate_gpus", return_value=gpus) as mock_enum:
    ...
    def test_mode_1_enabled_stays_on_when_no_gpus(self):
        ...
        with patch("optimum_hub_services.gpu.enumerate_gpus", return_value=[]) as mock_enum:
            result = resolve_gpu_mode(1)
        assert result == (1, 0, [])
        mock_enum.assert_called_once_with()
...
class TestEnumerateGpus:
    ...
    def test_empty_when_no_gpus_reported(self):
        with patch("optimum_hub_services.gpu_client.fetch_payload_with_retry", return_value=_payload([])):
            assert enumerate_gpus() == []
