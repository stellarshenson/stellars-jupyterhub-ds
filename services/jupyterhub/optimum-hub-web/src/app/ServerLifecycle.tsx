...
export function ServerLifecycleProvider({ children }: { children: ReactNode }) {
  ...
  const start = useCallback((user: string) => { void runOp(user, 'start', startServer, (s) => s === 'active' || s === 'idle') }, [runOp])
  ...
  return <Ctx.Provider value={{ start, restart, stop, busyOf }}>{children}</Ctx.Provider>
}
