...
from stellars_hub.gpu import detect_nvidia, enumerate_gpus, resolve_gpu_mode


class TestResolveGpuMode:
    def test_mode_0_disabled(self):
        """Mode 0 returns (0, 0, []) without touching the GPU container."""
        with patch("stellars_hub.gpu.enumerate_gpus") as mock_enum:
    ...
    def test_mode_1_enabled_enumerates(self):
        ...
        with patch("stellars_hub.gpu.enumerate_gpus", return_value=gpus) as mock_enum:
    ...
    def test_mode_1_enabled_stays_on_when_no_gpus(self):
        ...
        with patch("stellars_hub.gpu.enumerate_gpus", return_value=[]) as mock_enum:
            result = resolve_gpu_mode(1, "nvidia/cuda:test")
        assert result == (1, 0, [])
        mock_enum.assert_called_once_with("nvidia/cuda:test")
...
class TestDetectNvidia:
    def test_present_when_gpus_enumerated(self):
        with patch("stellars_hub.gpu.enumerate_gpus", return_value=[{"index": "0"}]):
            assert detect_nvidia("nvidia/cuda:test") == 1

    def test_absent_when_no_gpus(self):
        with patch("stellars_hub.gpu.enumerate_gpus", return_value=[]):
            assert detect_nvidia("nvidia/cuda:test") == 0
