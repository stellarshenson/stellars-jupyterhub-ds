...
A higher-fidelity React build of the Optimum Hub JupyterHub portal on Ant Design Pro. It reads real data read-only from the hub REST API; every mutating action is mocked (toast only, no write method exists). This is the canonical frontend artefact - it supersedes the static `../mock/` HTML prototype that drove the original design exploration.
...
- **Footer** - Optimum Hub + JupyterHub versions as tags on one line with the JupyterHub / JupyterLab / Ant Design Pro stack chips
